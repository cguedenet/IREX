...
library(pacman)
p_load(tidyverse, leaflet, RColorBrewer, rgdal, sp,here)
```
...
#explore data attributes in shapefile
combined@polygons@Polygons@coords
class(combined)
...
jormap <- jormap %>%
  addPolygons(stroke = FALSE,
              smoothFactor = 0.2,
              fillOpacity = .5,
              fillColor = viridis(12,alpha=NULL)) # creates a vector of n contiguous color
  
# add lines
jormap <- jormap %>% addPolylines(color = "grey",
                        weight = .5)

# add legend
jormap %>% addLegend("bottomleft",
                     color = topo.colors(12),
